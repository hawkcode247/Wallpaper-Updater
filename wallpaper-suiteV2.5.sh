...
echo $$ > "$SUITE_LOCK"
trap 'rm -f "$SUITE_LOCK"' EXIT INT TERM
...
ui_ask(){ center; case "$GUI" in
    ...
    *) local a=""; printf '%b' "$1" >&2; read -rp " [Y/n]: " a || a=""; [[ ! "${a,,}" =~ ^n ]];; esac; }
...
ui_pick(){ local text="$1" def="$2"; shift 2; center; case "$GUI" in
    ...
    *) printf '\n%b\n' "$(sed -E 's/<[^>]+>//g' <<<"$text")" >&2; local i=1 tags=() t l
        ...
        local c=""; read -rp "Choose [1-${#tags[@]}]: " c || c=""
        [[ "$c" =~ ^[0-9]+$ && "$c" -ge 1 && "$c" -le ${#tags[@]} ]] && echo "${tags[c-1]}" || echo "$def";; esac; }
...
ui_text(){ center; case "$GUI" in
    ...
    *) local v=""; read -rp "$1 [$2]: " v || v=""; echo "${v:-$2}";; esac; }
        ...
        (
            setsid bash -c '
                ...
                while IFS= read -r line; do
                    case "$line" in
                        *ActionInvoked*(uint32\ '"$nid"',\ \x27view\x27)*)
...
EOF_LOCKSCREEN
    ...
    chmod 755 "$SCRIPTS_DIR/spotlight.sh" "$SCRIPTS_DIR/lockscreen.sh" 2>/dev/null
    ...
    if [[ "$(id -u)" -eq 0 ]] || command -v sudo &>/dev/null; then
        ...
        $CMD_PREFIX chmod 755 /usr/local/bin/spotlight.sh /usr/local/bin/lockscreen.sh 2>/dev/null || true
    fi

    bash -n "$SCRIPTS_DIR/spotlight.sh" && bash -n "$SCRIPTS_DIR/lockscreen.sh" \
...
act_install(){
    ...
    if [[ "$(id -u)" -eq 0 ]] || command -v sudo &>/dev/null; then
        ...
        # System-level GDM/Lockscreen startup boot service (runs before display manager GUI loads!)
        $CMD_PREFIX mkdir -p /etc/systemd/system 2>/dev/null || true
        $CMD_PREFIX cat >/etc/systemd/system/lockscreen-boot.service <<EOF
[Unit]
Description=Pre-greeter Lockscreen and Login Page Image Updater
...
[Service]
Type=oneshot
ExecStart=/bin/bash /usr/local/bin/lockscreen.sh next --force
TimeoutStartSec=120
...
EOF
        ...
        $CMD_PREFIX systemctl enable lockscreen-boot.service 2>/dev/null || true
...
wipe(){
    ...
    if [[ "$(id -u)" -eq 0 ]] || command -v sudo &>/dev/null; then
        ...
        $CMD_PREFIX rm -f /usr/local/bin/spotlight.sh /usr/local/bin/lockscreen.sh 2>/dev/null || true
