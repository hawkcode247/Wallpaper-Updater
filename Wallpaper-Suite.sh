...
VERSION="2026-08-01"
PAYLOAD_HASH="20260801loginfix3"
...
trap 'cleanup_suite; exit 130' INT TERM
...
dialog_dims(){
    local want_w="${1:-620}" want_h="${2:-420}" sw=0 sh=0 out maxw maxh
    DIALOG_W="$want_w"; DIALOG_H="$want_h"
    if [[ -n "${DISPLAY:-}" ]] && command -v xdotool &>/dev/null; then
        read -r sw sh < <(xdotool getdisplaygeometry 2>/dev/null || echo "0 0")
        ...
        [[ -n "$out" ]] && read -r sw sh <<< "$out"
    fi
    if [[ "$sw" =~ ^[0-9]+$ && "$sh" =~ ^[0-9]+$ && $sw -gt 0 && $sh -gt 0 ]]; then
        maxw=$(( sw * 88 / 100 )); maxh=$(( sh * 82 / 100 ))
        (( DIALOG_W > maxw )) && DIALOG_W="$maxw"
        (( DIALOG_H > maxh )) && DIALOG_H="$maxh"
        (( DIALOG_W < 460 )) && DIALOG_W=460
        (( DIALOG_H < 260 )) && DIALOG_H=260
    fi
}
ui_info_details(){
    local summary="$1" details="$2" out rc
    dialog_dims 680 500
    center
    case "$GUI" in
        zenity)
            if zenity --help-general 2>/dev/null | grep -q -- '--extra-button'; then
                while :; do
                    out="$(zenity --info --title="$APP_TITLE" --width="$DIALOG_W" --height="$DIALOG_H"                         --text="$summary" --ok-label=OK --extra-button="Applied list" 2>/dev/null)"; rc=$?
                    [[ $rc -ne 0 ]] && return 0
                    if [[ "$out" == "Applied list" ]]; then
                        printf '%b' "$details" | zenity --text-info --title="$APP_TITLE — Applied list"                             --width="$DIALOG_W" --height="$DIALOG_H" --ok-label=Back 2>/dev/null || return 0
                    else
                        return 0
                    fi
                done
            else
                zenity --info --title="$APP_TITLE" --width="$DIALOG_W" --height="$DIALOG_H"                     --text="$summary
Applied list:
$details" 2>/dev/null || true
            fi ;;
        kdialog)
            kdialog --title "$APP_TITLE" --msgbox "$summary
Applied list:
$details" 2>/dev/null || true ;;
        yad)
            yad --title="$APP_TITLE" --center --width="$DIALOG_W" --height="$DIALOG_H"                 --text="$summary
<b>Applied list</b>
$details" --button=OK 2>/dev/null || true ;;
        *) printf '
%b
Applied list:
%b
...
ui_info(){ center; case "$GUI" in
    zenity) zenity --info --title="$APP_TITLE" --width=560 --text="$1" 2>/dev/null;;
    kdialog) kdialog --title "$APP_TITLE" --msgbox "$1" 2>/dev/null;;
    yad) yad --title="$APP_TITLE" --center --width=560 --text="$1" --button=OK 2>/dev/null;;
    *) printf '\n%b\n' "$1";; esac; }
ui_err(){ dlog "UIERR: $1"; center; case "$GUI" in
    zenity) zenity --error --title="$APP_TITLE" --width=560 --text="$1" 2>/dev/null;;
    kdialog) kdialog --title "$APP_TITLE" --error "$1" 2>/dev/null;;
    yad) yad --title="$APP_TITLE" --center --image=dialog-error --width=560 --text="$1" --button=OK 2>/dev/null;;
    *) printf '\nERROR: %b\n' "$1" >&2;; esac; }
ui_ask(){ center; case "$GUI" in
    zenity) zenity --question --title="$APP_TITLE" --width=560 --text="$1" 2>/dev/null;;
    kdialog) kdialog --title "$APP_TITLE" --yesno "$1" 2>/dev/null;;
    yad) yad --title="$APP_TITLE" --center --width=560 --text="$1" --button=Yes:0 --button=No:1 2>/dev/null;;
...
ui_pick(){ local text="$1" def="$2"; shift 2; center; case "$GUI" in
    zenity) local a=() t l o
        ...
        zenity --list --radiolist --title="$APP_TITLE" --width=640 --height=400 --text="$text" \
            --column= --column= --column= --hide-header --hide-column=2 --print-column=2 "${a[@]}" 2>/dev/null || return 130;;
    kdialog) local a=() t l o
        while (($#)); do t="$1"; l="$2"; shift 2; o=off; [[ "$t" == "$def" ]] && o=on; a+=("$t" "$l" "$o"); done
        kdialog --title "$APP_TITLE" --radiolist "$text" "${a[@]}" 2>/dev/null || return 130;;
    yad) local a=() t l o out
        while (($#)); do t="$1"; l="$2"; shift 2; o=FALSE; [[ "$t" == "$def" ]] && o=TRUE; a+=("$o" "$t" "$l"); done
        out="$(yad --list --radiolist --title="$APP_TITLE" --center --width=640 --height=400 --text="$text" \
            --column= --column= --column= "${a[@]}" 2>/dev/null)" || return 130
        cut -d'|' -f2 <<<"$out";;
    *) printf '\n%b\n' "$(sed -E 's/<[^>]+>//g' <<<"$text")" >&2; local i=1 tags=() t l
        while (($#)); do t="$1"; l="$2"; shift 2; tags+=("$t"); printf '  %d) %s\n' "$i" "$l" >&2; i=$((i+1)); done
        local c=""; read -rp "Choose [1-${#tags[@]}, q = cancel]: " c || return 130; c="${c//$'\r'/}"
        if [[ "$c" =~ ^[Qq]$ ]]; then return 130
        elif [[ "$c" =~ ^[0-9]+$ && "$c" -ge 1 && "$c" -le ${#tags[@]} ]]; then echo "${tags[c-1]}"
        elif [[ -z "$c" ]]; then echo "$def"
        else return 130; fi;; esac; }
ui_text(){ center; case "$GUI" in
    zenity) zenity --entry --title="$APP_TITLE" --width=560 --text="$1" --entry-text="$2" 2>/dev/null || return 130;;
    kdialog) kdialog --title "$APP_TITLE" --inputbox "$1" "$2" 2>/dev/null || return 130;;
    yad) yad --entry --title="$APP_TITLE" --center --width=560 --text="$1" --entry-text="$2" 2>/dev/null || return 130;;
    *) local v=""; read -rp "$1 [$2] (q = cancel): " v || return 130; v="${v//$'\r'/}"; [[ "$v" =~ ^[Qq]$ ]] && return 130; echo "${v:-$2}";; esac; }
ui_password(){ center; case "$GUI" in
    zenity) zenity --password --title="$APP_TITLE" --width=560 2>/dev/null || return 130;;
    kdialog) kdialog --title "$APP_TITLE" --password "$1" 2>/dev/null || return 130;;
    yad) yad --entry --hide-text --title="$APP_TITLE" --center --width=560 --text="$1" 2>/dev/null || return 130;;
    *) local v=""; printf '%b' "$1" >&2; read -rsp " Password: " v || return 130; printf '\n' >&2; echo "$v";; esac; }
ui_busy(){
    local label="$1"; shift
    dlog "RUN: $*"
    if [[ -t 1 ]]; then
...
ensure_sudo_session(){
    [[ "${EUID:-$(id -u)}" -eq 0 ]] && { SUDO_AUTH_READY=1; return 0; }
    command -v sudo &>/dev/null || { ui_err "Administrator rights are required for the pre-login boot/login-screen setup, but sudo is not installed."; return 1; }
    if [[ "${SUDO_AUTH_READY:-0}" == "1" ]] && sudo -n true 2>/dev/null; then
        start_sudo_keepalive; return 0
    fi
    if sudo -n true 2>/dev/null; then
        SUDO_AUTH_READY=1; start_sudo_keepalive; return 0
    fi
    dlog "requesting one sudo authentication for complete setup"
    if [[ -t 0 ]]; then
        printf '
%s
' "Administrator password is needed once for the whole setup (system boot service, login screen, and global timers)." >&2
        sudo -v || return 1
    else
        local pw rc
        pw="$(ui_password "Administrator password is needed once for the whole setup.
It will be used only to unlock sudo, then discarded.")" || return 130
        printf '%s
' "$pw" | sudo -S -p '' -v >/dev/null 2>&1
        rc=$?
        pw=""; unset pw
        if [[ $rc -ne 0 ]]; then
            ui_err "Administrator authentication failed or was cancelled. Setup stopped before making system changes."
            return $rc
        fi
    fi
    SUDO_AUTH_READY=1
...
sudo_run(){
    if [[ "${EUID:-$(id -u)}" -eq 0 ]]; then
        ...
        [[ "${SUDO_AUTH_READY:-0}" == "1" ]] || ensure_sudo_session || return 1
        sudo -n "$@"
    fi
}
sudo_write_file(){
    local dst="$1" mode="${2:-644}" tmp rc
    tmp="$(mktemp "${TMPDIR:-/tmp}/wallpaper-suite.XXXXXX")" || return 1
    cat > "$tmp"
    sudo_run install -m "$mode" "$tmp" "$dst"
    rc=$?
...
extract(){
    if [[ -f "$HASH_MARK" && -f "$SCRIPTS_DIR/spotlight.sh" && -f "$SCRIPTS_DIR/lockscreen.sh" \
          && -f "$SCRIPTS_DIR/boot.sh" && -f "$SCRIPTS_DIR/login-apply.sh" ]] \
...
import_graphical_env_from_pid() {
    local pid="$1" key val
    [[ -r "/proc/$pid/environ" ]] || return 1
    while IFS='=' read -r key val; do
        case "$key" in
            DISPLAY|WAYLAND_DISPLAY|XAUTHORITY|XDG_CURRENT_DESKTOP|XDG_SESSION_DESKTOP|DESKTOP_SESSION|XDG_SESSION_TYPE|DBUS_SESSION_BUS_ADDRESS)
                [[ -n "$val" ]] || continue
...
' < "/proc/$pid/environ" 2>/dev/null)
    [[ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}${DBUS_SESSION_BUS_ADDRESS:-}" ]]
}
hydrate_graphical_env() {
    ...
    for comm in gnome-shell plasmashell kwin_wayland kwin_x11 cinnamon mate-session xfce4-session lxqt-session lxsession sway Hyprland gnome-session-binary; do
        while IFS= read -r pid; do
            [[ -n "$pid" ]] || continue
            import_graphical_env_from_pid "$pid" && return 0
        done < <(pgrep -u "$uid" -x "$comm" 2>/dev/null || true)
    done
    return 1
}
have_graphical_session() {
    hydrate_graphical_env || true
    [[ -n "${DISPLAY:-}${WAYLAND_DISPLAY:-}" ]] && return 0
...
wait_graphical_session() {
    local i
    for i in {1..60}; do
        have_graphical_session && return 0
        sleep 1
    done
    return 1
}
set_wallpaper() {
    local img="$1" record_change="${2:-1}" env applied=1
    hydrate_graphical_env || true
    ...
    if [[ $applied -eq 0 ]]; then
        ...
        if [[ "$record_change" != "0" ]]; then
            ...
            systemctl --user restart spotlight.timer 2>/dev/null || true
        fi
        return 0
    else
...
if [[ "$DO_PREDOWNLOAD" != "1" && -f "$SPOTLIGHT_PATH/predownloaded" && -f "$SPOTLIGHT_PATH/background.jpg" ]]; then
    if ! wait_graphical_session; then
        log notice "Boot-predownloaded wallpaper is ready, but no graphical user session is available yet — preserving marker."
        RUN_OK=1
        exit 0
    fi
    ...
    if (( $(date +%s) - _pm <= INTERVAL_LIMIT )); then
        ...
        if set_wallpaper "$SPOTLIGHT_PATH/background.jpg" 0; then
        ...
        log warning "Graphical session was detected, but applying the boot-predownloaded wallpaper failed — preserving marker."
        RUN_OK=1
        exit 0
    else
        rm -f "$SPOTLIGHT_PATH/predownloaded"
...
if [[ "$FORCE" == "0" && -f "$LAST_CHANGE_FILE" && -f "$SPOTLIGHT_PATH/background.jpg" ]]; then
    ...
    if [[ "$last_time" =~ ^[0-9]+$ ]]; then
        ...
        if (( elapsed < INTERVAL_LIMIT && elapsed >= 0 )); then
            if ! wait_graphical_session; then
                log notice "Interval not elapsed and no graphical session is available — keeping current wallpaper and not fetching a second image."
                RUN_OK=1
                exit 0
            fi
            log info "Interval not elapsed yet (${elapsed}s / ${INTERVAL_LIMIT}s). Re-applying current desktop wallpaper only."
            if set_wallpaper "$SPOTLIGHT_PATH/background.jpg" 0; then
                RUN_OK=1
                exit 0
            fi
            log warning "Could not re-apply current desktop wallpaper; not fetching a second image before the interval."
            RUN_OK=1
            exit 0
        fi
    fi
...
Commands:
  ...
  apply-current    Re-apply the current login image to the user's lock screen (no download)
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        next|apply-current|install|reinstall|uninstall) CMD="$1"; shift ;;
...
run_update() {
    ...
    date +%s > "$LAST_CHANGE_FILE" 2>/dev/null || true
    if [[ "$(id -u)" -eq 0 ]]; then
        systemctl try-restart wallpaper-suite-lockscreen.timer 2>/dev/null || true
    else
        systemctl --user restart lockscreen.timer 2>/dev/null || true
    fi
}
apply_current() {
    trap 'rm -f "$TMP_IMG"' EXIT
    if [[ -f "$IMG_PATH" ]]; then
        apply_lockscreen "$IMG_PATH" || warn "no known lock-screen mechanism found — current image is at $IMG_PATH"
        echo "Lock screen current image applied: $IMG_PATH ($(image_resolution "$IMG_PATH"))"
        exit 0
    fi
    warn "no current lock-screen image found at $IMG_PATH — waiting for boot/system timer"
    exit 0
}
case "$CMD" in
    install)        do_install ;;
...
RUN_DIR="/run/wallpaper-suite"
mkdir -p "$RUN_DIR" 2>/dev/null || true
BOOT_DONE="$RUN_DIR/boot.done"
if [[ -f "$BOOT_DONE" ]]; then
    log "boot updater already completed this boot — skipping duplicate run"
    exit 0
fi
touch "$BOOT_DONE" 2>/dev/null || true
...
exit 0
EOF_BOOT
    cat << 'EOF_LOGIN_APPLY' > "$SCRIPTS_DIR/login-apply.sh"
#!/usr/bin/env bash
set -u
if [[ -x "$HOME/.spotlight/spotlight.sh" ]]; then
    exec bash "$HOME/.spotlight/spotlight.sh" next --wait-net
elif [[ -x /usr/local/bin/spotlight.sh ]]; then
    exec bash /usr/local/bin/spotlight.sh next --wait-net
fi
exit 0
EOF_LOGIN_APPLY
    chmod 755 "$SCRIPTS_DIR/spotlight.sh" "$SCRIPTS_DIR/lockscreen.sh" "$SCRIPTS_DIR/boot.sh" "$SCRIPTS_DIR/login-apply.sh" 2>/dev/null
    bash -n "$SCRIPTS_DIR/spotlight.sh" && bash -n "$SCRIPTS_DIR/lockscreen.sh" && bash -n "$SCRIPTS_DIR/boot.sh" && bash -n "$SCRIPTS_DIR/login-apply.sh" \
        || { ui_err "Extraction failed verification.\nLog: $LOG"; return 1; }
    if [[ "$(id -u)" -eq 0 ]] || [[ "${SUITE_PRIVILEGED:-0}" == "1" ]]; then
        sudo_run mkdir -p /usr/local/bin 2>/dev/null || true
        sudo_run cp -f "$SCRIPTS_DIR/spotlight.sh" /usr/local/bin/spotlight.sh 2>/dev/null || true
        sudo_run cp -f "$SCRIPTS_DIR/lockscreen.sh" /usr/local/bin/lockscreen.sh 2>/dev/null || true
        sudo_run cp -f "$SCRIPTS_DIR/boot.sh" /usr/local/bin/wallpaper-boot.sh 2>/dev/null || true
        sudo_run cp -f "$SCRIPTS_DIR/login-apply.sh" /usr/local/bin/wallpaper-suite-login 2>/dev/null || true
        sudo_run chmod 755 /usr/local/bin/spotlight.sh /usr/local/bin/lockscreen.sh /usr/local/bin/wallpaper-boot.sh /usr/local/bin/wallpaper-suite-login 2>/dev/null || true
        ...
        dlog "skipping /usr/local/bin copy (no privileged setup/cached sudo)"
    fi
...
read_suite_intervals(){
    SUITE_WI=150
    SUITE_LI=240
    local ICONF="$CONFIG_DIR/suite.conf"
    if [[ -f "$ICONF" ]]; then
        while IFS='=' read -r _k _v; do
            case "$_k" in
                WALLPAPER_INTERVAL)  [[ "$_v" =~ ^[0-9]+$ && "$_v" -ge 5 ]] && SUITE_WI="$_v" ;;
                LOCKSCREEN_INTERVAL) [[ "$_v" =~ ^[0-9]+$ && "$_v" -ge 5 ]] && SUITE_LI="$_v" ;;
            esac
        done < "$ICONF"
    fi
    SUITE_WI_STR="$(fmt_interval "$SUITE_WI")"
    SUITE_LI_STR="$(fmt_interval "$SUITE_LI")"
}
units(){
    mkdir -p "$UNIT_DST"
    read_suite_intervals
    set_conf_key "$CONFIG_DIR/config" INTERVAL_MIN "$SUITE_WI"
    set_conf_key "${XDG_CONFIG_HOME:-$HOME/.config}/lockscreen/config" INTERVAL_MIN "$SUITE_LI"
...
[Unit]
Description=Apply boot-predownloaded desktop wallpaper at login, then run on schedule

[Timer]
OnStartupSec=1s
OnUnitInactiveSec=$SUITE_WI_STR
Persistent=false
...
[Unit]
Description=Sync current login image to user lock-screen settings (no download)
After=graphical-session.target

[Service]
Type=oneshot
ExecStart=/usr/bin/env bash $exec_lockscreen apply-current
Environment=XDG_RUNTIME_DIR=%t
TimeoutStartSec=120
EOF
    cat >"$UNIT_DST/lockscreen.timer" <<EOF
[Unit]
Description=Re-apply current login image to the user lock screen at login and on schedule

[Timer]
OnStartupSec=5s
OnUnitInactiveSec=$SUITE_LI_STR
Persistent=false
...
write_login_autostart(){
    ...
    cat > "$adir/wallpaper-suite-login.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=Wallpaper Suite Login Apply
Comment=Apply the boot-predownloaded wallpaper after graphical login
Exec=/usr/local/bin/wallpaper-suite-login
Terminal=false
NoDisplay=true
X-GNOME-Autostart-enabled=true
EOF
    chmod 644 "$adir/wallpaper-suite-login.desktop" 2>/dev/null || true
}
...
act_install(){
    ...
    if ! CAT="$(ui_pick "Choose your wallpaper style:" default \
        default  "🌐  Default — Spotlight · Bing · NASA APOD · Wallhaven · Picsum" \
        science  "🔬  Scientific — NASA library · research & tech imagery" \
        wildlife "🦜  Wildlife Sanctuary — animals · sanctuaries · nature")"; then
        return 0
    fi
    [[ "$CAT" =~ ^(default|science|wildlife)$ ]] || return 0
    if ui_ask "Keep previous wallpapers in ~/.wallpaper?\n(You can reuse them later)"; then
        if ! c="$(ui_pick "Archive size limit:" mb500 \
            mb500 "500 MB — recommended" gb1 "1 GB" custom "Custom size…")"; then
            return 0
        fi
        case "$c" in
            gb1) L=1024;;
            custom)
                if ! v="$(ui_text "Limit (e.g. 750MB or 2GB):" 750MB)"; then return 0; fi
                v="${v^^}"; v="${v// /}"
    ...
    if [[ "$(id -u)" -ne 0 ]] && ! command -v sudo &>/dev/null; then
        ui_err "Administrator rights are required for the pre-login boot/login-screen setup, but sudo is not installed."
        return 1
    fi
    if [[ "$(id -u)" -ne 0 ]] && ! sudo -n true 2>/dev/null; then
        ui_info "Setup needs administrator permission once.\n\nNo wallpaper will be applied during installation. The first real boot will update the login screen before the GUI starts and predownload the desktop wallpaper for instant login."
    fi
    ...
    S+="\n✔ Configuration saved  ($CAT · archive: $( ((A)) && echo "${L} MB" || echo off ))"
    S+="\n✔ No image was applied during installation (prevents false boot/double-change)"
    S+="\n✔ Graphical-login autostart installed as backup for instant user-session apply"
    dlog "Setting up system-wide defaults, pre-login boot service, and login-screen timer"
    sudo_run mkdir -p /etc/skel/.spotlight /etc/skel/.config/systemd/user /etc/skel/.config/autostart /etc/systemd/user /etc/systemd/system 2>/dev/null || true
    sudo_run cp -f "$SCRIPTS_DIR/spotlight.sh" "$SCRIPTS_DIR/lockscreen.sh" "$SCRIPTS_DIR/login-apply.sh" /etc/skel/.spotlight/ 2>/dev/null || true
    sudo_write_file /etc/skel/.config/autostart/wallpaper-suite-login.desktop 644 <<EOF
[Desktop Entry]
Type=Application
Name=Wallpaper Suite Login Apply
Comment=Apply the boot-predownloaded wallpaper after graphical login
...
X-GNOME-Autostart-enabled=true
EOF
    sudo_run chmod 755 /etc/skel/.spotlight/*.sh 2>/dev/null || true
    local u
    for u in spotlight.service spotlight.timer lockscreen.service lockscreen.timer; do
        sudo_run cp -f "$UNIT_DST/$u" /etc/skel/.config/systemd/user/ 2>/dev/null || true
        sudo_run cp -f "$UNIT_DST/$u" /etc/systemd/user/ 2>/dev/null || true
    done
    sudo_write_file /etc/systemd/system/lockscreen-boot.service 644 <<EOF
[Unit]
Description=Wallpaper Suite pre-login boot updater
...
EOF
    sudo_write_file /etc/systemd/system/wallpaper-suite-lockscreen.service 644 <<EOF
[Unit]
Description=Wallpaper Suite scheduled login-screen updater
After=network-online.target NetworkManager.service network.target
Wants=network-online.target
ConditionPathExists=/usr/local/bin/lockscreen.sh

[Service]
Type=oneshot
ExecStart=/bin/bash /usr/local/bin/lockscreen.sh next --wait-net
TimeoutStartSec=300
EOF
    sudo_write_file /etc/systemd/system/wallpaper-suite-lockscreen.timer 644 <<EOF
[Unit]
Description=Wallpaper Suite login-screen updater timer

[Timer]
OnBootSec=$SUITE_LI_STR
OnUnitInactiveSec=$SUITE_LI_STR
Persistent=false
Unit=wallpaper-suite-lockscreen.service

[Install]
WantedBy=timers.target
EOF
    sudo_run systemctl daemon-reload 2>/dev/null || true
    sudo_run systemctl stop lockscreen-boot.service wallpaper-suite-lockscreen.service 2>/dev/null || true
    if sudo_run systemctl enable lockscreen-boot.service >/dev/null 2>&1; then
        S+="\n✔ Pre-login boot updater enabled (runs on next real boot before display-manager)"
    else
        S+="\n⚠ Pre-login updater enable failed — run: sudo systemctl enable lockscreen-boot.service"
    fi
    if sudo_run systemctl enable wallpaper-suite-lockscreen.timer >/dev/null 2>&1; then
        S+="\n✔ Login-screen schedule enabled (first scheduled run after $SUITE_LI_STR on next boot)"
    else
        S+="\n⚠ Login-screen schedule enable failed — run: sudo systemctl enable wallpaper-suite-lockscreen.timer"
    fi
    sudo_run systemctl --global enable spotlight.timer lockscreen.timer 2>/dev/null || true
    S+="\n✔ System-wide support configured (new users inherit scripts/timers)"
    if have_sysd; then
        local E=""
        systemctl --user daemon-reload 2>>"$LOG" || true
        systemctl --user disable --now spotlight.service lockscreen.service 2>>"$LOG" || true
        systemctl --user enable spotlight.timer 2>>"$LOG" || E+=" spotlight.timer"
        systemctl --user enable lockscreen.timer 2>>"$LOG" || E+=" lockscreen.timer"
        systemctl --user stop spotlight.service lockscreen.service 2>/dev/null || true
        [[ -z "$E" ]] && S+="\n✔ User timers enabled but not started now (no immediate image change)" \
                      || S+="\n✘ User timer enable failed:$E\n   Fix after login: systemctl --user enable spotlight.timer lockscreen.timer"
    else
        S+="\n⚠ User timers are ready — activate after login without --now:\n   systemctl --user enable spotlight.timer lockscreen.timer"
    fi
    S+="\n✔ Next real boot flow: login image changes before GUI; desktop image is predownloaded and applied at login; timers handle later changes."
    local SUMMARY="Install finished.\n\nPlease reboot once to complete activation. On the next real boot, the login screen will update before the GUI loads and the user's desktop wallpaper will apply immediately after login.\n\nLog: $LOG\n\nClick “Applied list” to view the installed/changed items."
    ui_info_details "$SUMMARY" "$S"
}
wipe(){
    if [[ "$(id -u)" -eq 0 ]] || command -v sudo &>/dev/null; then
        ensure_sudo_session || true
    fi
    systemctl --user disable --now spotlight.service spotlight.timer lockscreen.service lockscreen.timer 2>/dev/null || true
    ...
    systemctl --user daemon-reload 2>/dev/null || true
    rm -f "${XDG_CONFIG_HOME:-$HOME/.config}/autostart/wallpaper-suite-login.desktop" 2>/dev/null || true
    ...
    rm -f "$HASH_MARK"
    if [[ "$(id -u)" -eq 0 ]] || command -v sudo &>/dev/null; then
        if [[ "${SUDO_AUTH_READY:-0}" == "1" ]] || ensure_sudo_session; then
            sudo_run systemctl disable --now lockscreen-boot.service wallpaper-suite-lockscreen.timer wallpaper-suite-lockscreen.service 2>/dev/null || true
            sudo_run rm -f /etc/systemd/system/lockscreen-boot.service \
                          /etc/systemd/system/wallpaper-suite-lockscreen.service \
                          /etc/systemd/system/wallpaper-suite-lockscreen.timer 2>/dev/null || true
            sudo_run rm -rf /etc/skel/.spotlight /etc/skel/.config/systemd/user/spotlight.* /etc/skel/.config/systemd/user/lockscreen.* /etc/skel/.config/autostart/wallpaper-suite-login.desktop 2>/dev/null || true
            sudo_run rm -f /etc/systemd/user/{spotlight,lockscreen}.{service,timer} 2>/dev/null || true
            sudo_run rm -f /usr/local/bin/spotlight.sh /usr/local/bin/lockscreen.sh /usr/local/bin/wallpaper-boot.sh /usr/local/bin/wallpaper-suite-login /var/log/wallpaper-boot.log 2>/dev/null || true
            sudo_run systemctl --global disable spotlight.timer lockscreen.timer 2>/dev/null || true
            sudo_run systemctl daemon-reload 2>/dev/null || true
        fi
    fi
}
...
while :; do
    ...
    if ! CH="$(ui_pick "<b>Wallpaper Suite</b>  v$VERSION\nStatus: $ST\n" "$DEF" \
